...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'scubad_refine_template.json'), 'r') as reader:
    scubad_refine_dict = json.load(reader)
...
def build_parser():
    ...
    parser.add_argument('--SCUBAD_root', type=str, required=True, help='SCUBAD_root')
...
def write_config(c_dict, json_f):
    with open(json_f, 'w') as write_f:
	    json.dump(c_dict, write_f, indent=4, ensure_ascii=False)
    
    
def refine_structure_from_scubdad(scubad_root, gen_dir):
    cur_path = os.getcwd()
    scubad_root = os.path.abspath(scubad_root)
    gen_dir = os.path.abspath(gen_dir)
    json_abspath_list = []
    for pdbf in os.listdir(gen_dir):
        if pdbf.endswith('_vqgen_from_indice.pdb'):
            pdbname = pdbf.split('.pdb')[0]
            cur_template_json_dict = deepcopy(scubad_refine_dict)
            cur_template_json_dict['global_config']['input_file'] = f'{gen_dir}/{pdbf}'
            cur_template_json_dict['global_config']['output_prefix'] = f'{gen_dir}'
            
            os.makedirs(f'{gen_dir}/scuba_d_json_f', exist_ok=True)
            json_f = f'{gen_dir}/scuba_d_json_f/{pdbname}.json'
            write_config(cur_template_json_dict, json_f)
            json_abspath_list.append(json_f)
    
    with open(f'{gen_dir}/scuba_d_json_list.txt', 'w') as writer:
        for line in json_abspath_list:
            writer.write(f'{line}\n')

    os.chdir(scubad_root)   
    logger.info(f'SCUBA-D: {scubad_root} refinement')
    command = [
        "python3.8", "inference_par.py", 
        "--test_list", f'{gen_dir}/scuba_d_json_list.txt',
        "--max_sample_num", '10000000',
        "--write_pdbfile",
        "--epoch_num", "1", 
        "--batch_size", "1",
    ...
    subprocess.run(command)
    os.chdir(f'{gen_dir}')
    os.system(f'/bin/rm -rf *.json')
    os.system(f'/bin/rm -rf scuba_d_json_f')
    os.system(f'/bin/rm -rf scuba_d_json_list.txt')
...
def main(args):
    ...
    data['contact_onehot'] = torch.zeros(protein_length).long()
    data['condition_mask'] = torch.zeros(protein_length).long()
    data['receptor_mask'] = torch.zeros(protein_length)
...
if __name__=='__main__':
    ...
    retrive_structure(args.decoder_root, args.decoder_param, gen_npy_dir)
    torch.cuda.empty_cache()
    refine_structure_from_scubdad(args.SCUBAD_root, gen_npy_dir)
