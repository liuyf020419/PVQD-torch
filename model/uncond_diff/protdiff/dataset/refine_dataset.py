...
class ProtDiffDataset(BaseDataset):
    ...
    def __getitem__(self, item:int):
        ...
        if self.train_mode:
            try:
                ...
                if self.global_config.loss_weight.prior_esm_single_pred_loss > 0.0:
                    test_esm_file = f'{self.config_data.esm1b_root}/{protein[1:3]}/{protein.split("_")[0]}_single_rep.npy'
                    test_esm_data = np.load(test_esm_file, allow_pickle=True).item()
                    assert test_esm_data.__contains__('esm_single_mask')
                loss_mask = False
            except:
            ...
            try:
                ...
                if self.global_config.loss_weight.prior_esm_single_pred_loss > 0.0:
                    esm_file = f'{self.config_data.esm1b_root}/{protein[1:3]}/{protein.split("_")[0]}_single_rep.npy'
                    esm_data = np.load(esm_file, allow_pickle=True).item()
                    unnorm_esm_single = esm_data['single_rep'] # L, D
                    ...
                    data_dict["esm_single_mask"] = torch.from_numpy(esm_data['esm_single_mask'])
                ...
                if not self.validate:
                    if len(data_dict['aatype']) >= self.max_len:
                        ...
                        crop_max_len = len(data_dict['aatype'])

                    multichain_mask = self.crop_contiguous(
                        [len(data_dict['aatype'])], crop_max_len)
                    if (len(data_dict['aatype']) == len(multichain_mask)):
                        pass
                    elif (len(data_dict['aatype']) > len(multichain_mask)):
                        mask_pad_num = len(data_dict['aatype']) - len(multichain_mask)
                        multichain_mask = np.concatenate([multichain_mask, np.zeros((mask_pad_num, ))])
                    elif (len(data_dict['aatype']) < len(multichain_mask)):
                        mask_crop_num = len(multichain_mask) - len(data_dict['aatype'])
                        multichain_mask = multichain_mask[:-mask_crop_num]
                    data_dict = self.crop_data(data_dict, torch.from_numpy(multichain_mask))
                    # data_dict = self.clamp_data(data_dict)
                else:
                    data_dict['reprint_resabsID'] = torch.from_numpy(tmpdata['reprint_resabsID']).long()
    ...
    def merge_pos_frame_data(self, data_dict: dict, integrate_dict: dict, traj_idx):
        ...
        # logger.info(f'integrate traj: {traj_idx}')
    ...
    def make_from_pdb_file(self, poteinfile, chain, datatype):
        ...
        if os.path.isfile(ss_config_f):
            ...
            traj_coords, traj_flat12s = permute_between_ss_from_pos(
                traj_pos, sstype[:, None], [5, 8], 0.0, self.config_data.white_noise.white_noise_scale, 
                self.config_data.white_ss_noise.sketch_data, ss_mask_p_range=[0, 0], loop_mask_p_range=[1.0, 1.0])
...
def permute_between_ss_from_pos(gt_pos, sstype, ca_noise_scale, quat_noise_scale, white_noise_scale, sketch_data, ss_mask_p_range, loop_mask_p_range):
    ca_pos = gt_pos[:, 1]
    ss3type = sstype[:, 0]
    ss_start_indexs = (torch.where((ss3type[1:] - ss3type[:-1]) != 0)[0] + 1).long()
    ss_start_indexs = torch.cat([torch.LongTensor([0]), ss_start_indexs])
