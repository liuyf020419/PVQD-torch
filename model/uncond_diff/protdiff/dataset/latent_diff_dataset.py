...
sys.path.append("/home/liuyf/proteins/SCUBA-diff-main-new-exp/pdb_utils/data_parser")
...
class ProtDiffDataset(BaseDataset):
    def __init__(self, config, data_list, train=True, pdbroot=None, \
        validate=False, permute_data=False, random_seed=None, multichain_inference=False):
        super().__init__()
        ...
        if self.train_mode:
            try:
                ...
                if data_mode == 'monomer':
                    ...
                    test_data_file = os.path.join(f'{dataroot}/{protein[1:3]}/{protein}_{chain_mask_str}/{protein}_{chain_mask_str}.npy')
                elif data_mode == 'complex':
                    dataroot = self.complex_dataroot
                    test_data_file = os.path.join(f'{dataroot}/{protein[1:3]}/{protein}/{protein}_{chain_mask_str}.npy')
            ...
            # try:
            if data_mode == 'monomer':
                data_file = os.path.join(f'{dataroot}/{protein[1:3]}/{protein}_{chain_mask_str}/{protein}_{chain_mask_str}.npy')
            else:
                data_file = os.path.join(f'{dataroot}/{protein[1:3]}/{protein}/{protein}_{chain_mask_str}.npy')
            ...
            data_dict['str_code'] = torch.from_numpy(coords_dict['str_code']).long()
            data_dict['protein_state'] = torch.tensor([0 if (data_mode == 'monomer') else (len(multichain_length_dict)-1)]).long()
    ...
    def crop_data(self, data_dict, multichain_mask):
        ...
        for name in data_dict.keys():
            ...
            elif name in ['max_len', 'protein_state']:
                new_data[name] = torch.LongTensor([data_dict[name]])
            elif name in [
                'traj_pos', 'gt_backbone_pos', 'traj_backbone_frame', 'gt_backbone_frame', 
    ...
    def merge_pos_frame_data(self, data_dict: dict, preprocess_crd_dict: dict, chain_mask):
        ...
        data_dict["gt_backbone_pos"] = gt_pos
