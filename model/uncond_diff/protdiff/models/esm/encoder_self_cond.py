...
class LatentEmbedder(nn.Module):
    def __init__(self, in_channels, hidden_size, config, self_condition) -> None:
        super().__init__()
        ...
        if self_condition:
            self.wtb = nn.Embedding(vocab_num, in_channels//2)
        else:
            self.wtb = nn.Embedding(vocab_num, in_channels)
        # self.wtb_projection = nn.Linear(config.codebook_dim, config.codebook_act_dim)
    ...
    def _init_embedding(self, pretrained_ckpt, norm_latent):
        ...
        wtb_weights = {k.replace('codebook.codebook_layer.0.', '') : v for k, v in weights_pkl['model'].items() if 'codebook.codebook_layer.0' in k}
        if norm_latent:
...
class DiTEncoder(nn.Module):
    def __init__(self, config, global_config, in_channels, out_channels=None)  -> None:
        ...
        self.x_embedder = LatentEmbedder(in_channels, hidden_size, global_config.latentembedder, global_config.self_condition)
    ...
    def initialize_weights(self):
        ...
        # for block in [self.t_embedder, self.y_embedder, self.blocks, self.final_layer]:
        #     block.apply(_basic_init) # besides x_embedder
