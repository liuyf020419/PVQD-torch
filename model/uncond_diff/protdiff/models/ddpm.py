...
from .nn_utils import make_mask, mask_loss, latent_loss, fape_loss_multichain, l2_distance_loss, get_coords_dict_from_affine, nll_loss
...
from .external_guide.fixbb_score_guide_utils import rmsd_gradient, update_x_with_grad, fape_gradient
...
class DDPM(nn.Module):
    ...
    def sampling_guidance(
        ...
        for t_idx in range(len(t_scheme)):
            ...
            # x_0_guide, traj_rmsd = rmsd_gradient(
            #     x0_dict['latent_rep'].clone(), pred_idx, target_coords[..., 1, :], target_idx, batch, guide_fn )

            # cur_scale = guide_decay_scheme(t[0]) * guide_scale
            # try:
            #     cur_scale = cur_scale.item()
            # except:
            #     pass
            # print(t_idx, traj_rmsd, cur_scale)
            # x_0_latent_rep = update_x_with_grad(x0_dict['latent_rep'], cur_scale, x_0_guide)
            # x0_dict['latent_rep'] = x_0_latent_rep
            ...
            try:
                ...
                pass
            print(t_idx, traj_rmsd, cur_scale)
            x_t_1_latent_rep = update_x_with_grad(x_t_1_dict['latent_rep'], cur_scale, x_t_1_guide)
