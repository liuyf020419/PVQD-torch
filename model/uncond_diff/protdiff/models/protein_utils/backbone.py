...
def backbone_fape_loss_multichain(
    ...
    # monomer_inter_mask = torch.any(chain_idx > 0, 1)
    intra_pair_mask = torch.where((chain_idx[:, None] - chain_idx[:, :, None]) == 0, 1., 0.)
    ...
    intra_loss = torch.sum(intra_dist_map * mask2d * intra_pair_mask) / (torch.sum(mask2d * intra_pair_mask) + 1e-6)
    intra_loss_clamp = torch.sum(intra_dist_map_clamp * mask2d * intra_pair_mask) / (torch.sum(mask2d * intra_pair_mask) + 1e-6)
    # inter clamp fape
    ...
    inter_loss = torch.sum(inter_dist_map * mask2d * inter_pair_mask) / (torch.sum(mask2d * inter_pair_mask) + 1e-6)
    inter_loss_clamp = torch.sum(inter_dist_map_clamp * mask2d * inter_pair_mask) / (torch.sum(mask2d * inter_pair_mask) + 1e-6)
