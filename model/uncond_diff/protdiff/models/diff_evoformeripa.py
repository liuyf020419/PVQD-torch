...
class EvoformerIPA(nn.Module):
    ...
    def forward(self, batch):
        ...
        device = xt_dict['affine'].device
        if not batch.__contains__('condition'):
            batch['condition'] = torch.zeros((batch_size, res_num)).to(device)
        ...
        if self.global_config.loss_weight.diff_esm_single_ce_loss > 0.0:
            ...
            # pred_aatype = self.esm_ce_head(single_rep)
