...
import sys
# sys.path.append('/home/liuyf/proteins/mc_vq_20230919/complex_vq/protdiff')
...
class LatentDiffModel(nn.Module):
    def __init__(self, config, global_config) -> None:
        ...
        if (encoder_mode == 'ESM2Encoder'):
            self.ldm = ESM2Encoder(config.ESM2Encoder, global_config, in_channels, in_channels)
            ...
            self.ldm = UViTEncoder(config.UViTEncoder, global_config, in_channels, in_channels)
        else:
            raise ValueError()
        ...
        # if ((self.global_config.loss_weight.fape_loss > 0.0) and self.training):
