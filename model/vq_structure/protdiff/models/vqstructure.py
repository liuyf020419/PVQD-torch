    ...
    make_low_resolution_mask, generate_new_affine, \
        fape_loss, make_mask, mask_loss, make_pairidx_from_singleidx, \
            distogram_loss, aatype_ce_loss, downsampling_single_idx, downsampling_pair_idx, \
                get_batch_quataffine, dihedral_loss, fape_loss_multichain, get_coords_dict_from_affine
...
class VQStructure(nn.Module):
    ...
    def forward(self, batch, return_all=False, use_codebook_num=4):
        ...
        if self.global_config.loss_weight.dihedral_diff_loss > 0.0:
            try:
                dihedral_diff_loss = dihedral_loss(
                    atom14_positions, batch['gt_pos'][..., :3, :], 
                    batch['single_mask'], batch['dihedral_mask'])
            except:
                dihedral_diff_loss = dihedral_loss(
                    atom14_positions, batch['gt_pos'][..., :3, :], 
                    batch['single_mask'])
            losses_dict.update({'dihedral_diff_loss': dihedral_diff_loss})
        ...
        if return_all:
            for batch_idx in range(batchsize):
                ...
                for chain_label in reduced_chain_idx:
                    ...
                    gt_coord4.append(add_atom_O( chain_coords.detach().cpu().numpy()[..., :3, :]).reshape(-1, 3))
            ...
            for batch_idx in range(batchsize):
                traj_coord_0 = []
                for chain_label in reduced_chain_idx:
                    chain_coords = coords_dict['coord'][-1, batch_idx][batch['chain_idx'][batch_idx] == chain_label]
                    traj_coord_0.append(
                        add_atom_O(chain_coords.detach().cpu().numpy()[..., :3, :]).reshape(-1, 3))
                ...
                if compute_sc_identity:
                    ...
                    for chain_label in reduced_chain_idx:
                        chain_coords = recycle_coords_dict['coord'][-1, batch_idx][batch['chain_idx'][batch_idx] == chain_label]
                        recycle_traj_coord_0.append(
                            add_atom_O(chain_coords.detach().cpu().numpy()[..., :3, :]).reshape(-1, 3))
                    write_multichain_from_atoms(recycle_traj_coord_0, 
                            f'{pdb_prefix}/{pdbname_list[batch_idx]}/{pdbname_list[batch_idx]}_recycle_vqrecon.pdb', natom=4)
                
                gt_aatype_str = []
                pred_aatype_str = []
                for chain_label in reduced_chain_idx:
                    chain_gt_aatype_str = ''.join([af2_index_to_aatype[aa] for aa in \
                        gt_aatype_af2idx[batch_idx][batch['chain_idx'][batch_idx] == chain_label].detach().cpu().numpy().tolist()])
                    gt_aatype_str.append(chain_gt_aatype_str)
                    chain_pred_aatype = pred_aatype[batch_idx][batch['chain_idx'][batch_idx] == chain_label]
                    chain_pred_aatype_logits = torch.argmax(chain_pred_aatype, -1).reshape((-1, )).detach().cpu().numpy()
                    chain_pred_aatype_str = ''.join([af2_index_to_aatype[aa] for aa in chain_pred_aatype_logits])
    ...
    def gen_structure_from_indices(
        ...
        pseudo_single_res_rel = torch.arange(res_num).long().to(device)[None]
        ...
        pair_res_rel_idx = torch.where(torch.any(torch.stack([pair_res_rel > pair_res_uplimit, 
                                pair_res_rel < -pair_res_uplimit]), 0), pair_res_mask_num, pair_res_rel)
        pseudo_pair_res_rel = pair_res_rel_idx.long() + pair_res_uplimit
        ...
        if save_dict:
            ...
            for chain_label in reduced_chain_idx:
                traj_coord_0.append(
                    add_atom_O(
                        coords_dict['coord'][-1, 0][chain_idx[0] == chain_label].detach().cpu().numpy()[..., :3, :]).reshape(-1, 3)
                    )
            if sequence is None:
                write_multichain_from_atoms(traj_coord_0, f'{prefix}_vqgen_from_indice.pdb', natom=4)
            else:
                write_multichain_from_atoms(traj_coord_0, f'{prefix}_vqgen_from_indice.pdb', aatype=[sequence], natom=4)

            pred_aatype_logits = torch.argmax(pred_aatype[-1], -1).reshape((-1, )).detach().cpu().numpy()
