...
class EdgeTransition(nn.Module):
    ...
    def forward(self, node_embed, edge_embed):
        ...
        if self.framediff_update:
            ...
            edge_bias = torch.cat([prod, diff], -1)
...
class InvariantPointAttention(nn.Module):
    ...
    def forward_checkpoint(
        ...
        if getattr(self._ipa_conf, "gradient_checkpointing", False) and self.training:
            a, o_pt, o = checkpoint(
                self.forward_attn, q, k, v, b, q_pts, k_pts, v_pts, mask, z, r, _offload_inference)
        else:
            a, o_pt, o = \
                self.forward_attn(q, k, v, b, q_pts, k_pts, v_pts, mask, z, r, _offload_inference)
    ...
    def forward_attn(self, q, k, v, b, q_pts, k_pts, v_pts, mask, z, r, _offload_inference):
        ...
        # return o, o_pt, o_pt_norm_feats, o_pair
...
class IPAAttention(nn.Module):
    ...
    def forward(self, init_node_embed, edge_embed, node_mask, rigids):
        node_mask = node_mask#.type(torch.float32)
        edge_mask = node_mask[..., None] * node_mask[..., None, :]
        init_frames = rigids#.type(torch.float32)
        ...
        for b in range(self._ipa_conf.num_blocks):
            ipa_embed = self.trunk[f'ipa_{b}'].forward_checkpoint(
                node_embed,
                edge_embed,
                curr_rigids,
                node_mask)
            ...
            if self._ipa_conf.skip_embed:
                if self._ipa_conf.skip_embed_init:
                    ...
                    skip_init = torch.ones_like(init_node_embed)
                seq_tfmr_in = torch.cat([
                    node_embed, self.trunk[f'skip_embed_{b}'](skip_init)
                ], dim=-1)
            else:
                seq_tfmr_in = node_embed
            if getattr(self._model_conf, "gradient_checkpointing", False) and self.training:
                seq_tfmr_out = checkpoint(
                    self.trunk[f'seq_tfmr_{b}'], 
                    seq_tfmr_in, None, 1 - node_mask
                )
            else:
