...
# from conv_block import UNet, STD_Resnet_block
# from . import rigid
# from .ipa_2d_net import EvoformerPairBlock_
...
class FoldBlock(nn.Module):
    ...
    def forward(self, 
        ...
        seq_mask = activations['single_mask']
        act = activations['act']
...
class AffineGenerator(nn.Module):
    ...
    def forward(self, representations, fix_region=None):
        ...
        activations = {'act': act,
                       ...
                       'single_mask': representations['single_mask'][:, :, None]
                      }
...
class StructureModule(nn.Module):
    def __init__(self, config, global_config, s_in_channel=None, p_in_channel=None):
        super().__init__()
        self.config = config
        self.global_config = global_config
        single_channel = config.single_channel
        pair_channel = config.pair_channel
        if s_in_channel is None:
            single_channel = config.single_channel
        else:
            single_channel = s_in_channel

        if p_in_channel is None:
            pair_channel = config.pair_channel
        else:
            pair_channel = p_in_channel
