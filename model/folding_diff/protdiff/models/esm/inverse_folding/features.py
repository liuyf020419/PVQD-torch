...
from .generate_random_graph import dist_noise
...
class GVPInputFeaturizer(nn.Module):
    ...
    @staticmethod
    def get_node_features(coords, coord_mask, with_coord_mask=True, all_atom=False, all_atom_mask=None):
        # scalar features
        ...
        if not all_atom:
            ...
            assert (all_atom_mask is not None)
            coords_to_ca = (coords - X_ca[..., None, :]) * all_atom_mask[..., None]
            node_vector_features = torch.cat([orientations, coords_to_ca], dim=-2)
    ...
    @staticmethod
    def _dist(X, coord_mask, res_idx, padding_mask, top_k_neighbors, eps=1e-8, order=3, mixed_nn=False, long_contact_top_k=None):
        """ Pairwise euclidean distances """
        bsz, maxlen = X.size(0), X.size(1)
        coord_mask_2D = torch.unsqueeze(coord_mask,1) * torch.unsqueeze(coord_mask,2)
        residue_mask = ~padding_mask # [1, ..., 1, 0, ..., 0]
        ...
        D_adjust = nan_to_num(D_cov) + (~coord_mask_2D) * (1e8 + Dseq*1e6) + (
            ~residue_mask_2D) * (1e10) # padding with 1e8 or 1e10
        D_out_adjust = nan_to_num(D) + (~coord_mask_2D) * (1e8 + Dseq*1e6) + (
            ~residue_mask_2D) * (1e10) # padding with 1e8 or 1e10
        ...
        if top_k_neighbors == -1:
            ...
            D_neighbors_ = torch.gather(D_out_adjust, -1, E_idx)
            # D_neighbors_ = torch.gather(D, -1, E_idx)

            if mixed_nn:
                if long_contact_top_k is None:
                    long_contact_top_k = 2 * top_k_neighbors
                long_topk = min(long_contact_top_k, X.size(1))
                E_noise_idx = dist_noise(D_adjust, long_topk, coord_mask_2D, residue_mask_2D, Dseq, E_idx)
                D_noise_neighbors = torch.gather(D_adjust, -1, E_noise_idx)
                merged_E_noise = torch.cat([E_idx, E_noise_idx], -1)
                merged_D_noise = torch.cat([D_neighbors, D_noise_neighbors], -1) # B, L, 2 * K
                merged_D_noise_idx = torch.argsort(merged_D_noise, -1, descending=False)
                E_idx = E_merge_idx = torch.gather(merged_E_noise, -1, merged_D_noise_idx)
                D_neighbors_ = D_merge_neighbors = torch.gather(D_out_adjust, -1, E_merge_idx)
    
        coord_mask_neighbors = (D_neighbors_ < 5e7)
        residue_mask_neighbors = (D_neighbors_ < 5e9)
        return D_neighbors_, E_idx, coord_mask_neighbors, residue_mask_neighbors
...
class GVPGraphEmbedding(GVPInputFeaturizer):
    ...
    def __init__(self, args):
        ...
        self.sidechain_rep = getattr(args, 'sidechain_rep', False)
        if (not self.sidechain_rep):
            node_input_dim = (7, 3)
            if getattr(self.args, "aatype_embed", False):
                self.ori_node_embed = nn.Linear(node_input_dim[0], 64)
                self.aatype_embedder = nn.Embedding(22, 64)
                node_input_dim = (128, 3)
            if getattr(self.args, "all_atom", False):
                ori_node_scaler, ori_node_vector = node_input_dim
                node_input_dim = (ori_node_scaler, 37+2)
        else:
            self.aatype_embedder = nn.Embedding(22, 64)
            node_input_dim = (64, 6)
    ...
    def forward(self, coords, coord_mask, res_idx, padding_mask, confidence, aatype=None, all_atom=False, all_atom_mask=None, mixed_nn=False):
        with torch.no_grad():
            if (not self.sidechain_rep):
                node_features = self.get_node_features(coords, coord_mask, all_atom=all_atom, all_atom_mask=all_atom_mask)
            else:
                assert (aatype is not None)
                node_features = self._get_sidechain_node_feature(aatype, coords)
            edge_features, edge_index = self.get_edge_features(
                coords, coord_mask, res_idx, padding_mask, self.edge_cross_vec, mixed_nn=mixed_nn)

        if (not self.sidechain_rep):
            if getattr(self.args, "aatype_embed", False):
                assert (aatype is not None)
                node_scaler_features, node_vec_features= node_features
                node_scaler_features = self.ori_node_embed(node_scaler_features)
                aatype_feature = self.aatype_embedder(aatype)
                node_scaler_features = torch.cat([node_scaler_features, aatype_feature], -1)
                node_features = (node_scaler_features, node_vec_features)
    ...
    def get_edge_features(self, coords, coord_mask, res_idx, padding_mask, edge_cross_vec=False, mixed_nn=False):
        X_ca = coords[:, :, 1]
        ...
        E_dist, E_idx, E_coord_mask, E_residue_mask = GVPInputFeaturizer._dist(
                ...
                mixed_nn=mixed_nn, long_contact_top_k=self.top_k_neighbors * 2)
    ...
    def _get_sidechain_node_feature(self, aatype, last_rigid_coords):
        node_scalar_features = self.aatype_embedder(aatype)
        # vector features
        X_center = last_rigid_coords[:, :, 1]
        orientations = get_rotation_frames(last_rigid_coords)

        coords_to_ca = (last_rigid_coords - X_center[..., None, :])
        node_vector_features = torch.cat([orientations, coords_to_ca], dim=-2)
