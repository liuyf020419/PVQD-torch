...
class DiTEncoder(nn.Module):
    def __init__(self, config, global_config, in_channels, out_channels=None, context_channels=None)  -> None:
        ...
        self.context_embedder = nn.Linear(context_channels, hidden_size)
        if getattr(self.config, "adaLN_context_embedder", False):
            self.adaLN_context_embedder = nn.Linear(context_channels, hidden_size)
    ...
    def initialize_weights(self):
        ...
        if getattr(self.config, "adaLN_context_embedder", False):
            nn.init.constant_(self.adaLN_context_embedder.weight, 0)
            nn.init.constant_(self.adaLN_context_embedder.bias, 0)


    def forward(self, x, t, y, single_mask, input_hidden=None, single_condition=None, context_condition=None):
        """
        Forward pass of DiT.
        ...
        if getattr(self.config, "adaLN_context_embedder", False):
            adaLN_context_condition = self.adaLN_context_embedder(context_condition).transpose(0, 1)
        context_condition = self.context_embedder(context_condition).transpose(0, 1)
        if single_condition is not None:
            x = x + single_condition.transpose(0, 1)
        ...
        for layer_idx, layer in enumerate(self.blocks):
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                x = checkpoint(
                    layer, 
                    x, c_, padding_mask, context_condition
                )
            else:
                x = layer(
                    x, c_, padding_mask, context_condition
        ...
        x = self.final_layer(x, c_).transpose(0, 1)     # (N, T, D)
        return x
...
class UViTEncoder(nn.Module):
    def __init__(self, config, global_config, in_channels, out_channels=None, context_channels=None):
        super().__init__()
