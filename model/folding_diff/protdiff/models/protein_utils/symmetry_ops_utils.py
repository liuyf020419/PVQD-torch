...
# def get_assemly_xt_from_au_xt_in_ori_new(au_xt_dict_in_ori, merged_chain_label, au_trans, au_rots):
...
#         # au_affine = au_affine[(merged_chain_label == 0)[..., None]]
#         au_affine = torch.stack([single_affine[(merged_chain_label == 0)[b_idx]] for b_idx, single_affine in enumerate(au_affine)])
#         # import pdb; pdb.set_trace()
...
#     assembly_xt_dict['esm'] = au_xt_dict_in_ori['esm']
...
def get_assemly_xt_from_au_xt_in_ori_new(au_xt_dict_in_ori, merged_chain_label, au_trans, au_rots):
    ...
    if au_xt_dict_in_ori.__contains__('affine'):
        au_affine = au_xt_dict_in_ori['affine']
        au_esm = au_xt_dict_in_ori['esm']
        # import pdb; pdb.set_trace()
        # au_affine = au_affine[(merged_chain_label == 0)[..., None]]
        au_affine = torch.stack([single_affine[(merged_chain_label == 0)[b_idx]] for b_idx, single_affine in enumerate(au_affine)])
        au_esm = torch.stack([single_esm[(merged_chain_label == 0)[b_idx]] for b_idx, single_esm in enumerate(au_esm)])
        # import pdb; pdb.set_trace()
        batch_size, res_num = au_affine.shape[:2]
        assemblly_affine_list = []
        assemblly_esm_list = []
        ...
        for au_idx in range(au_num):
            ...
            assemblly_affine_list.append(au_sym_affine)
            assemblly_esm_list.append(au_esm)
        # import pdb; pdb.set_trace()
        assemblly_affine = torch.cat(assemblly_affine_list, -2)
        assembly_xt_dict['affine'] = assemblly_affine.float()
        assemblly_esm = torch.cat(assemblly_esm_list, -2)
        assembly_xt_dict['esm'] = assemblly_esm.float()
