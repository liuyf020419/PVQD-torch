...
def find_structural_violations_group(
    ...
    # batch_size = batch['single_mask'].shape[0]
    # traj_num = 1
    # import pdb; pdb.set_trace()
    ...
    # else:
    #     tmp_batch = {'seq_mask': batch['single_mask'].repeat(traj_num, 1),
    #                 'single_res_rel': batch['single_res_rel'].repeat(traj_num, 1)}
    tmp_batch = {'seq_mask': batch['single_mask'],
                'single_res_rel': batch['single_res_rel']}

    violations = find_structural_violations_batch(tmp_batch, atom14_positions, config, weight)
    return violations


def structural_violation_loss(batch, atom14_positions, config, weight=None):
  """Computes loss for structural violations."""
#   dtype = atom14_positions.dtype
  if weight is not None:
      assert len(weight.shape) == 2
      violation_weight = weight[..., None]
  else:
      weight = violation_weight = 1.0
    
  violations = find_structural_violations_group(batch, atom14_positions, config, weight)

  num_atoms = np.prod(list(atom14_positions.shape[:-1])).astype(np.float32)
  violations['between_residues']['clashes_per_atom_loss_sum'] = violations['between_residues']['clashes_per_atom_loss_sum'] * violation_weight
  violations['within_residues']['per_atom_loss_sum'] = violations['within_residues']['per_atom_loss_sum'] * violation_weight
  violation_loss = (
