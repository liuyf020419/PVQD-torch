...
def dihedral_loss(pred_coords, gt_coords, single_mask):
    ...
    dihedral_mse_reduce = torch.sum(diff_dihedral) / (torch.sum(single_mask) + 1e-6)
    # import pdb; pdb.set_trace()
...
def l2_distance_loss(l2_distance, gt_code, single_mask, mode='inverse', weight=None, compute_loss=False):
    l2_distance_loss_dict = {}
    batchsize, num_res, str_code_num = l2_distance.shape
    if mode == 'inverse':
        l2_distance_new = 1 / l2_distance
    else:
        l2_distance_new = -l2_distance
    if compute_loss:
        l2_dist_loss = F.cross_entropy(l2_distance_new.reshape(-1, str_code_num), gt_code.reshape(-1), reduction='none').reshape(batchsize, num_res)
        l2_dist_loss = l2_dist_loss * single_mask
        if weight is not None:
            l2_dist_loss = l2_dist_loss * weight[:, None]
        l2_dist_loss_reduce = torch.sum(l2_dist_loss) / (torch.sum(single_mask) + 1e-6)
        l2_distance_loss_dict['l2_distance_loss'] = l2_dist_loss_reduce

    pred = torch.argmax(l2_distance_new, dim=-1)
    acc = torch.sum((pred == gt_code).float() * single_mask) / (torch.sum(single_mask) + 1e-6)
    l2_distance_loss_dict['l2_distance_acc'] = acc

    return l2_distance_loss_dict


def latent_loss(pred_latent, true_latent, single_mask, weight=None, return_dict=True):
    latent_loss_dict = {}
    latent_dim = pred_latent.shape[-1]
    latent_single_error = F.mse_loss(pred_latent, true_latent, reduction='none') # B, L, D
    latent_single_mask = (single_mask)[..., None].repeat(1,1,latent_dim)
    latent_single_error = latent_single_error * latent_single_mask
    if weight is not None:
        latent_single_error = latent_single_error * weight[:, None, None]

    loss = torch.sum(latent_single_error, dim=[-2, -1]) / (torch.sum(latent_single_mask, dim=[-2, -1]) + 1e-6)
    latent_loss_dict['latent_mse_loss'] = loss
    if return_dict:
...
def nll_loss(pred_logits, true_aatype, single_mask, weight=None):
    latent_loss_dict = {}
    batchsize, num_res, vocab_num = pred_logits.shape
    aatype_nll_loss = F.cross_entropy(pred_logits.reshape(-1, vocab_num), true_aatype.reshape(-1), reduction='none').reshape(batchsize, num_res)
    aatype_nll_loss = aatype_nll_loss * single_mask
    if weight is not None:
        aatype_nll_loss = aatype_nll_loss * weight[:, None]
    aatype_nll_loss_reduce = torch.sum(aatype_nll_loss) / (torch.sum(single_mask) + 1e-6)
    latent_loss_dict['nll_loss'] = aatype_nll_loss_reduce
    pred = torch.argmax(pred_logits, dim=-1)
    acc = torch.sum((pred == true_aatype).float() * single_mask) / (torch.sum(single_mask) + 1e-6)
    latent_loss_dict['nll_acc'] = acc
    return latent_loss_dict


def sc_simi_loss(pred_latent, codebook_rep, gt_code, single_mask, weight=None):
    latent_loss_dict = {}
    batchsize, num_res, latent_dim = pred_latent.shape
    code_num = codebook_rep.shape[0]

    similarity = torch.matmul(pred_latent, codebook_rep[None].transpose(1, 2))
    similarity_ce_loss = F.cross_entropy(similarity.reshape(-1, code_num), gt_code.reshape(-1), reduction='none').reshape(batchsize, num_res) # B, L

    similarity_ce_loss = similarity_ce_loss * single_mask
    if weight is not None:
        similarity_ce_loss = similarity_ce_loss * weight[:, None, None]
    loss = torch.sum(similarity_ce_loss) / (torch.sum(single_mask) + 1e-6)
    latent_loss_dict['sidechain_simi_loss'] = loss
