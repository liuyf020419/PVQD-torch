...
import numpy as np
import math
from .esm.modules import MultiheadAttention
...
class DiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, use_rotary_embeddings=True, cross_attn=False):
        ...
        if self.cross_attn_flag:
            self.cross_attn_norm = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        ...
        if self.cross_attn_flag:
            self.cross_attn_adaLN_modulation = nn.Sequential(
            nn.SiLU(),
            nn.Linear(hidden_size, 3 * hidden_size, bias=True)
        )

    def forward(self, x, c, single_mask, context=None):
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(c).chunk(6, dim=-1)
        shift_c_attn, scale_c_attn, gate_c_attn = self.cross_attn_adaLN_modulation(c).chunk(3, dim=-1)
    ...
    def attn(self, x, self_attn_mask=None, self_attn_padding_mask=None, need_head_weights=False, use_flash_attention = False):
        x, _ = self.self_attn(
            query=x,
            key=x,
            value=x,
